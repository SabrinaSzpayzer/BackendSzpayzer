...
app.get('/info', (req, res) => {
    ...
    const path = process.execPath; //otra opción: const path = __filename;
    const proceso = process.pid;
    const directorio = process.cwd();
    res.render('info', {argumentos: argumentos, plataforma: plataforma, versionNode: versionNode, memoria: memoria, path: path, proceso: proceso, directorio: directorio});
});
...
controllersdb(config.URL_BASE_DE_DATOS, err => {
    ...
    const connectedServer = httpServer.listen(minimist(process.argv.slice(2), PORT), () => {
